    ...
    def construct_rounds(self):
        ...
        for i in range(l - 1):
            ...
            for j in range(start, end, step):
                ...
                i2 = k
                target = j + step // 2
                #print("target",target,j,step ,step // 2,n)
                if target > n:#n
                    pass
                else:
                    # print(i1, i2, target)
        ...
        for k in range(2, l):
            ...
            span = 2 ** (k - 1)
            # print("#############c-round##########")
            print("start, step, span", start, step, span)
            # print("############################")
            for p in range(start, n, step):
                ...
                if p + span > n:
                    ...
                    print("c_round:", "a:p",p,"b:o1",o1,"a[p+span]:",p + span)
                    o1 += 2
            o1 = o2
            o2 += (o2 - 1) // 2
        print("l",l)
        ...
        last_round = [cirq.CNOT(self.B[0], ancillae00) ]
        last_round += [cirq.CNOT(self.B[i + 1], ancillae1[i]) for i in range(n -1)]
        last_round += [cirq.CNOT(self.A[0], ancillae00) ]
        last_round += [cirq.CNOT(self.A[i + 1], self.B[i+1]) for i in range(n - 1)]
