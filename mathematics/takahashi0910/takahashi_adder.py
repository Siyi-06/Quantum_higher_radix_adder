...
class TakahashiAdder:
    def __init__(self, nr_qubits, A, B, ancillae=None, type = True):
        self._qubit_order = []
        self.A = A
        self.B = B
        self.size = len(A)
        ...
        for i in range(nr_qubits):
            self._qubit_order.append(self.B[i])
            self._qubit_order.append(self.A[i])
        self._qubit_order.append(self.ancillae)
